...
func (c *ChannelConn) PollRW(ctx context.Context) (bool, error) {
	subCtx, cancel := context.WithCancel(ctx)
	defer cancel()

	var bar *sync.Cond = sync.NewCond(&sync.Mutex{})

	var ready atomic.Bool
	var err atomic.Value // error

	go func(ret *atomic.Bool) {
		defer bar.Signal()

		r, rErr := c.PollR(subCtx)
		if r {
			ret.Store(true)
		} else {
			err.Store(rErr)
